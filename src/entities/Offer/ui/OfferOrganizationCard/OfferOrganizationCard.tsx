...
export const OfferOrganizationCard: FC<OfferOrganizationCardProps> = memo((
    ...
    const { organization: { name, description } } = props;
    return (
            ...
            <div className={styles.container}>
                <div className={styles.fullInfoContainer}>
                    <div className={styles.nameContainer}>
                        <img className={styles.image} src={organizationDefaultImage} alt="organization" />
                        <span className={styles.name}>{name}</span>
                    </div>
                    <p className={styles.description}>{description}</p>
                </div>
                <Button className={styles.button} variant="outlined">Подробнее</Button>
            </div>
        </div>
    );
});
