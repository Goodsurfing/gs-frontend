...
export const AddOffer: FC = () => {
    ...
    const addOfferHandle = async () => {
        if (!myProfile?.hostId || !myProfile.isActive) {
            return;
        }
        ...
        try {
            ...
            navigate(getOffersWelcomePageUrl(locale, result.id.toString()));
        } catch {
    ...
    useEffect(() => {
        setDisabledButton(!myProfile?.hostId || !myProfile.isActive);
    }, [myProfile?.hostId, myProfile?.isActive]);
